...
int main(int argc, char* argv[])
{
    ...
    while(true) {
        ...
        char daytime_msg[daytime.size() + 1];
        strcpy(daytime_msg, daytime.c_str());
        sockaddr_in * client_addr = new sockaddr_in;
        socklen_t len = sizeof (sockaddr_in);
        int bytes_received = recvfrom(s, connect, sizeof(connect), 0, (sockaddr*)(client_addr), &len);
        if(bytes_received < 0) {
            cout << "Error receiving from client\n";
        } else {
            cout << "Successful client connection!\n";
            int bytes_sent = sendto(s, daytime_msg, strlen(daytime_msg), 0, (sockaddr*)(client_addr), len);
            if(bytes_sent < 0) {
                cout << "Error sending daytime to client\n";
            } else {
